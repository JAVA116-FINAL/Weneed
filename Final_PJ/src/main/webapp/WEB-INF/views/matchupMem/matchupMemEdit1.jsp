	...
	$.getJikmubt=function(jList){
		...
		if(jList.length>0){
			$.each(jList, function(idx,item){
				...
				jikmu+="<input type='checkbox' name='mcujikmuList["+idx+"].jikmuCode'class='jikmuList' id='ckjikmu"+item.jikmuCode+"' value='"+item.jikmuCode+"'/>"+item.jikmuName    
				jikmu+="</label>";    
			...
			<div class="col-lg-9 col-md-6 machupWrapperdiv_jy paddingTop_jy">
				<div class="doctor-details mt-4 mt-lg-0 machupinner_jy">
					<div class="machupSection_jy">
						<form id="frmMcuEdit1" class="appoinment-form" method="post" action="<c:url value='/matchupMem/matchupMemEdit1.do'/>">
						...
						<!-- 직군선택하기  forEach로 뿌리기-->				
				                ...
				                <!-- 전문가번호 -->  
				                            	 <input type="hidden" name="expertVo.expertiseNo" value="${expertVo.expertiseNo }">
				                <!-- 직군코드 -->
				                                <select class="form-control mcuEditFormctr_jy" id="exampleFormControlSelect1" name="expertVo.jikgunCode">
				                                 <option value="0">선택하세요</option>
				                                  <c:forEach var="jikgunVo" items="${jikgunList}">
					                              	  <option value="${jikgunVo.jikgunCode}" <c:if test="${expertVo.jikgunCode==jikgunVo.jikgunCode}"> selected="selected"</c:if>>${jikgunVo.jikgunName}</option>
				                                  </c:forEach>
				                                </select>
				                            </div>
						                  </div>
											        ...
											        <div class="btn-group btn-group-toggle mcuEdit-btgruop_jy" data-toggle="buttons" id="divbtGroup">
											        	<c:set var="i" value="0"></c:set>
											          	<c:forEach var="jikmuVo" items="${jikmuList}">
														    <c:forEach var="mjikmuVo" items="${mJikmuList}">
														    	<c:if test="${mjikmuVo.jikmuCode eq jikmuVo.jikmuCode }">
														     <%-- 		<input type="text" value="${mjikmuVo.jikmuCode}" class="mJikmuList"> --%>
														     		<label class="btn mcuEdit-jikgunbt_jy active">
																        <input type="checkbox" class="jikmuList" name="mcujikmuList[${i}].jikmuCode" checked="checked" value="${jikmuVo.jikmuCode}">${jikmuVo.jikmuName }
																 	 </label>
						    ...
						    <!-- 경력들어가야함 -->   
							    			<div class="jobarea_jy mcuEditjabarea_jy">	
						            		  ...
						            		  <!--경력-->            
					                                <select class="form-control mcuEditFormctr_jy" name="expertVo.career" id="career">
					                                  <option value="신입" <c:if test="${'신입'eq expertVo.career }">selected="selected"</c:if>>신입</option>
					                                  <option value="1" <c:if test="${'1' eq expertVo.career}">selected="selected"</c:if>>1년</option>
					                                  <option value="2" <c:if test="${'2' eq expertVo.career}">selected="selected"</c:if>>2년</option>
					                                  <option value="3" <c:if test="${'3' eq expertVo.career}">selected="selected"</c:if>>3년</option>
					                                  <option value="4" <c:if test="${'4' eq expertVo.career}">selected="selected"</c:if>>4년이상</option>
					                                  <option value="5" <c:if test="${'5' eq expertVo.career}">selected="selected"</c:if>>5년이상</option>
					                                  <option value="6" <c:if test="${'6' eq expertVo.career}">selected="selected"</c:if>>6년이상</option>
					                                  <option value="7" <c:if test="${'7' eq expertVo.career}">selected="selected"</c:if>>7년이상</option>
					                                  <option value="8" <c:if test="${'8' eq expertVo.career}">selected="selected"</c:if>>8년이상</option>
					                                  <option value="9" <c:if test="${'9' eq expertVo.career}">selected="selected"</c:if>>9년이상</option>
					                                  <option value="10이상" <c:if test="${'10이상' eq expertVo.career}">selected="selected"</c:if>>10년이상</option>
					                                </select>
						                         </div>
											</div>
										</div>
										...
										<div class="divformblock-contents_jy">
											...
											<!-- 연봉 -->
														<span class="spSalary_jy" id="spSalary_jy">
															<input type="text" name="expertVo.salary" readonly="readonly" id="salary" placeholder="선택하기" value="${expertVo.salary}">
														</span>
											...
											<!-- 스킬 -->	
														<span class="spSkills_jy" id="spSkills_jy">
															<input type="text" name="expertVo.skill" readonly="readonly" id="skill" placeholder="선택하기" value="${expertVo.skill }">
														</span>
																	...
																	<div class="modalbodyinnderDiv_jy">
																		<div class="modalDivSelect1_jy">
																			...
																			<div class="skillinputDiv" onClick="$.setSkills()">																	
																				<input hidden="hidden" >
																				<input type="text" name="skillsinput_jy" id="skillsinput_jy" placeholder="보유기술을 작성해주세요" value="">
								...
								<div class="machupMemEdit-btOk_jy mcuRegisterdiv">
									<button type="submit" class="btMachupEdit_jy mcuRegisterbt">
										<span class="spbtText">수정완료</span> <!-- 이때. 전문가번호, 직무List,매치업번호 insert=>이력서 인서트한다음에 매치업에 업뎃 -->
									</button>
								</div> 
							</form> 	
						</div>
