...
public class MatchupMemServiceImpl implements MatchupMemService{
	...
	public int insertMcuMem(MatchupMemVO mVo) {
		return matchupMemDao.insertMcuMem(mVo);
	}
	...
	public List<Map<String, Object>> selectSearchedMemList(MatchupMemSearchVO mcuMemSearchVo){ 
		//키워드, 사이즈, 넘버리스트가 들어가야 할거 같구나
		
		//매치업번호 리스트 합칠 것 
		List<Integer> mcumemNoList=new ArrayList<Integer>();
		
		//커리어의 매치업번호 리스트 테스트 전
		List<Integer> careerMcumemNoList=careerDao.selectMcumemNo(mcuMemSearchVo.getKeyword());
		System.out.println("커리어 검색결과 careerMcumemNoList.size="+careerMcumemNoList.size());

		//에듀의 매치업번호 리스트 테스트 전
		List<Integer> eduMcumemNoList=educationDao.selectMcumemNo(mcuMemSearchVo.getKeyword());
		System.out.println("에듀 검색결과 eduMcumemNoList.size="+eduMcumemNoList.size());
		
		//익스퍼트의 매치업번호 리스트 테스트 전
		List<Integer> expertMcumemNoList=expertDao.selectMcumemNo(mcuMemSearchVo.getKeyword());
		...
		mcuMemSearchVo.setMcumemList(mcumemNoList);
		//이제 매치업넘버리스트는 완성됐어 이거에 대한 이력서리스트를 가져와야 해
		List<Map<String, Object>> mcumemSearchResultList=matchupMemDao.selectMcumemSearchList(mcuMemSearchVo);
